    ...
    def __init__(self, args):
        ...
        for i in range(0,args.I):
            ...
            for p in range(0, args.P):
                ...
                self.mid_supplier_inventory[i][p] = self.mid_supplier_inventory[i][p]*args.invent
        ...
        for k in range(args.K):
            for j in range(args.J):
                temp1 += self.demand[k][0][j]
                temp2 += self.demand[k][1][j]
                
        ### ------------------- production line ----------------- ###
        self.num_production_line = args.pnn*self.num_production_line
        ...
        if(args.temp == 1):
            args.K = 1 
            for j in range(args.J):
                self.demand[0][0][j] = self.demand[args.No][0][j]
                self.demand[0][1][j] = self.demand[args.No][1][j]
...
class master():
    ...
    def run_eval(self,args,s,x,m,v):
        ...
        name = args.result_path + "/" + str(args.model) + "_k_" + str(args.K) + "e_" + str(args.error) + "_" + str(args.cut) + "_modular_" + str(args.modular) + "_" + str(args.UR) + "_" + str(args.output_name) + "_TIU_" + str(args.TIU) + "_sf_" + str(int(args.short_factor*10)) + "_df_" + str(int(args.dep_factor*10)) + "_mis_" + str(int(args.mis_factor*10)) + "_un_" + str(int(args.unused_factor*10)) + "__" + str(int(args.No)) +  ".csv"
        df.to_csv(name)
