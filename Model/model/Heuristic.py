...
class master():
    def __init__(self,args,input_data):
        ...
        self.model.addConstr(self.theta == quicksum(self.input_data.area_region_distance[w][j]*self.input_data.house_volumn_factor[p]*self.input_data.trans_price*self.fk_wj[w,j,p,t] for w in range(args.W) for j in range(args.J) for p in range(args.P) for t in range(self.input_data.first_end_time_point,args.T))
                ...
                + quicksum((0.8*args.emergency_price_factor*self.input_data.house_price[p] + self.input_data.supplier_region_distance[i][j]*self.input_data.house_volumn_factor[p]*self.input_data.trans_price)*self.fk_ij[i,j,p,t] for i in range(args.I) for j in range(args.J) for t in range(self.input_data.first_end_time_point,args.T) for p in range(args.P)))
    ...
    def run(self,args):
        ...
        for k in range(o_argsK):
            for e in range(2):
                ...
                if(e ==0):
                    obj_normal = obj_normal + self.model.ObjVal
                    first_stage = first_stage + self.model.ObjVal - self.theta.x
                    second_stage = second_stage + self.theta.x
                    ...
                    deprivation += sum((self.input_data.deprivation_a0[0] + self.input_data.deprivation_a1[0]*(args.TIU)*(args.T-t))*(self.input_data.house_price[g+1])*(self.qk[j,g,t].x) for j in range(args.J) for t in range(self.input_data.deprivation_time_point,args.T) for g in range(args.G))
                    ...
                    obj_emergency = obj_emergency + self.model.ObjVal
        ...
        print("perfect/Normal)",obj_normal/o_argsK)
        #print("first_stage)",first_stage/o_argsK)
        #print("second_stage)",second_stage/o_argsK)
        #print("penalty_cost)",(unmet+unused+deprivation)/o_argsK)
        #print("unmet)",unmet/o_argsK)
        #print("unused)",unused/o_argsK)
        #print("deprivation)",deprivation/o_argsK)
        #print("emergency_cost)",emergency_cost/o_argsK)
