...
class input_data:
    def __init__(self, args):
        ...
        second_end_time_point_TIU = int(math.floor(self.second_end_time_point/args.TIU))
        deprivation_time_point_TIU = int(math.floor((self.deprivation_time_point)/args.TIU))
        ...
        if(args.temp == 1):
            self.demand_per_trailer = np.reshape(self.demand_per_trailer , (-1, args.J))
            self.demand_per_MHU = np.reshape(self.demand_per_MHU , (-1, args.J))
...
class sub_N():
    def __init__(self,args,input_data):
        ...
        if(args.model == "TSCC_without"):
            self.sub_Normal.setObjective(quicksum(args.short_factor*1000000*self.input_data.house_price[g+1]*(qk[j,g,args.T-1]) for j in range(args.J) for g in range(args.G))+quicksum(args.dep_factor*(self.input_data.deprivation_a0[0] + self.input_data.deprivation_a1[0]*(args.TIU)*(args.T-t))*1000000*(qk[j,g,t]) for j in range(args.J) for t in range(self.input_data.deprivation_time_point,args.T) for g in range(args.G)),GRB.MINIMIZE)
            ...
            self.sub_Normal.setObjective(quicksum(self.input_data.area_region_distance[w][j]*self.input_data.house_volumn_factor[p]*self.input_data.trans_price*fk_wj[w,j,p,t] for w in range(args.W) for j in range(args.J) for p in range(args.P) for t in range(self.input_data.first_end_time_point,args.T))
                ...
                + quicksum(args.unused_factor*self.input_data.unused[p]*self.input_data.house_price[p]*mk[w,p,args.T-1] for w in range(args.W) for p in range(args.P))
                + quicksum(args.dep_factor*(self.input_data.deprivation_a0[0] + self.input_data.deprivation_a1[0]*(args.TIU)*(args.T-t))*(self.input_data.house_price[g+1]/120)*(qk[j,g,t]) for j in range(args.J) for t in range(self.input_data.deprivation_time_point,args.T) for g in range(args.G))
    ...
    def run_evaluation(self,args,k,x_vals,m_vals,v_vals,new_dir_path):
        ...
        deprivation = sum(args.dep_factor*(self.input_data.deprivation_a0[0] + self.input_data.deprivation_a1[0]*(args.TIU)*(args.T-t))*(self.input_data.house_price[g+1]/120)*(self.qk[j,g,t].x) for j in range(args.J) for t in range(self.input_data.deprivation_time_point,args.T) for g in range(args.G))
...
class sub_R():
    ...
    def run_evaluation(self,args,k,s_vals,x_vals,m_vals,v_vals,new_dir_path):
        ...
        unused3 = sum((self.mk[w,2,args.T-1].x) for w in range(args.W))
        deprivation = sum(args.dep_factor*(self.input_data.deprivation_a0[0] + self.input_data.deprivation_a1[0]*(args.TIU)*(args.T-t))*(self.input_data.house_price[g+1]/120)*(self.qk[j,g,t].x) for j in range(args.J) for t in range(self.input_data.deprivation_time_point,args.T) for g in range(args.G))
        emergency_cost = sum((args.emergency_price_factor*self.input_data.supplier_price[i][p] + self.input_data.supplier_region_distance[i][j]*self.input_data.house_volumn_factor[p]*self.input_data.trans_price)*self.fk_ij[i,j,p,t].x for i in range(args.I) for j in range(args.J) for t in range(self.input_data.first_end_time_point,args.T) for p in range(args.P))
...
class master():
    def __init__(self,args,input_data,sub_N,sub_R,index):
        ...
        self.theta_0 = self.model.addVars(args.K, vtype=GRB.CONTINUOUS, name='theta0')
        self.theta_1 = self.model.addVars(args.K, vtype=GRB.CONTINUOUS, name='theta1')
        ...
        self.model.addConstr(self.theta >= quicksum((1-self.z[k])*self.theta_0[k] + self.z[k]*self.theta_1[k] for k in range(args.K))/args.K)
    ...
    def check_MIPSOL(self,args,model,where):
        if where == GRB.Callback.MIPSOL:
            ...
            theta_vals = model.cbGetSolution(model._thetavar)
            theta0_vals = model.cbGetSolution(model._theta0var)
            theta1_vals = model.cbGetSolution(model._theta1var)
           ...
           # pdb.set_trace()
            ...
            if(args.cut == "bilinear" or args.cut == "all" or args.cut =="two"):
                
                start = time.time()
                for k in range(args.K):
                    
                    if(theta0_vals[k] < dual_opt[0][k] - cut_vio_thred and abs(theta0_vals[k] - dual_opt[0][k])/max(abs(theta0_vals[k]),1e-10) > cut_vio_thred):

                        model.cbLazy(model._theta0var[k] >= (quicksum(pi_2h[0][k][j][g][t]*self.input_data.demand[k][g][j] for g in range(args.G) for j in range(args.J) for t in range(args.T-self.input_data.first_end_time_point) if(zvars_round[k] == 0))
                    ...
                    if(theta1_vals[k] < dual_opt[1][k] - cut_vio_thred and abs(theta1_vals[k] - dual_opt[1][k])/max(abs(theta1_vals[k]),1e-10) > cut_vio_thred):

                        model.cbLazy(model._theta1var[k] >= (quicksum(pi_2h[1][k][j][g][t]*self.input_data.demand[k][g][j] for j in range(args.J) for g in range(args.G) for t in range(args.T-self.input_data.first_end_time_point) if(zvars_round[k] == 1))
                                                    ...
                                                    + quicksum(self.input_data.extra_production_line[i]*pi_4i[1][k][i][t] for i in range(args.I) for t in range(args.T-self.input_data.first_end_time_point) if (zvars_round[k] == 1)) ))
                ...
                global generate_bilinear_cut_time
                generate_bilinear_cut_time = generate_bilinear_cut_time + (end - start)


            if(args.cut == "bigM" or args.cut == "both" or args.cut == "all"):
            ...
            if(args.cut == "Spe" or args.cut == "both" or args.cut == "all" or args.cut =="two" ):
                ...
                if(theta_vals < lower_bound - cut_vio_thred and abs(theta_vals - lower_bound)/max(abs(theta_vals),1e-10) > cut_vio_thred):
                    ...
                    for k in range(args.K):
                        if(abs(z_vals[k]) <= 1e-5):
                            ...
                            end1 = time.time()
                            temp1 = end1 - start1
                            solving_sub_Emergency_problem = solving_sub_Emergency_problem + end1 - start1
                            ...
                            end0 = time.time()
                            temp0 = end0 - start0
                            solving_sub_Normal_problem = solving_sub_Normal_problem + end0 - start0
                    ...
                    if(args.cut_save == 1):
                        ...
                        global cut_count
                        path_temp =  "cut/{}_{}_{}_cut_{}_{}_{}_{}.npz".format(args.K,args.error,args.TIU,cut_count,int(args.short_factor*10),int(args.dep_factor*10),int(args.unused_factor*10))
    ...
    def run_eval(self,args,s,x,m,v):
        ...
        for k in range(args.K):
            ...
            if(args.model == "TSCC_De_Sp_class" and args.Large_Scenario_main == 0):
                ...
                elif(abs(self.z[k].x-1) <= nonint_ratio):
                    output_index.append(k)
        ...
        if(args.threshold_policy == 1):
            ...
            name = args.result_path + "/" + "e_" + str(args.error) + ".txt"
            f = open(name,"w+")
            f.write("threshold number: %s \r" % (threshold_em_ac_num))
    ...
    def run(self,args):
        ...
        self.model._theta0var = self.theta_0
        self.model._theta1var = self.theta_1
        ...
        if(args.Heu == 1):
            ...
            self.model.write("heu/{}_{}_{}_{}_{}_{}heu_solution.sol".format(args.K,args.error,args.TIU,int(args.short_factor*10),int(args.dep_factor*10),int(args.unused_factor*10)))
            end = time.time()
            ...
            for count in range(cut_count):
                ...
                path_temp =  "cut/{}_{}_{}_cut_{}_{}_{}_{}.npz".format(args.K,args.error,args.TIU,count,int(args.short_factor*10),int(args.dep_factor*10),int(args.unused_factor*10))
            ...
            self.model.read("heu/{}_{}_{}_{}_{}_{}heu_solution.sol".format(args.K,args.error,args.TIU,int(args.short_factor*10),int(args.dep_factor*10),int(args.unused_factor*10)))
        ...
        global generate_bigM_cut_time
        global generate_special_cut_time
        global solving_sub_Normal_problem
        global solving_sub_Emergency_problem
        global solving_master_problem
        global big_M_cut
        global spe_cut
        global node
        global generate_bilinear_cut_time
        global MIP_solution_time
        ...
        MIP_solution_time = self.model.cbGet(GRB.Callback.RUNTIME)
        ...
        if(args.model == "TSCC_without"):
            args.model = "TSCC_De_Sp_class"
            self.sub_N = sub_N(args,self.input_data)
