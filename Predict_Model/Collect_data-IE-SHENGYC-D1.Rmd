...
ggplot(moran.df, aes(x = distance, y = moran_I, col=model)) + geom_point() + geom_line()
```
...
ggplot(moran.df, aes(x = distance, y = moran_I, col=model)) + 
  geom_point() + 
  geom_line()  +
  theme_bw(base_size = 12) + theme(legend.box.background = element_rect(color="black", size=1),legend.position = c(0.6, 0.7)) +  guides() + xlab("Number of Nearest Neighbors") + ylab("Moran's I") + guides(color = guide_legend(title = "Regression Model")) 

ggsave("type1_regression.pdf")
```
